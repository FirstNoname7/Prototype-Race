...
{
    //этот скрипт нужен для того, чтоб камера следовала завторым игроком (за машиной), когда она начнёт двигаться. Чтобы игрок видел то, что видит машина (только сверху)
    ...
    private Vector3 offsetOn = new Vector3(0, 2, 1); //второе положение камеры (когда мы на месте водителя)
    ...
    {
        if (Input.GetKeyDown(KeyCode.P)) //если нажать на кнопку Р, тогда:
        {
